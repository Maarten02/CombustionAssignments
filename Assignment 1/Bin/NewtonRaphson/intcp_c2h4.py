...
def intcpc2h4(phi, T0, Tad, Tr):
    ...
    for id in reac_ids:
        for coef in coefs2[id]:
            acoefs_reac_lt += coef * massfracs_r[id] * R_bar[id]
    ...
    if not stop:
        ...
        for id in reac_ids:
            for coef in coefs1[id]:
                acoefs_reac_ht += coef * massfracs_r[id] * R_bar[id]

        for i, supcoef_ht in enumerate(acoefs_reac_ht):
            intR += supcoef_ht / (i + 1) * (Tr ** (i + 1) - T0 ** (i + 1))
    ...
    for id in prod_ids:
        for coef in coefs2[id]:
            acoefs_prod_lt += coef * massfracs_r[id] * R_bar[id]

    for i, supcoef_lt in enumerate(acoefs_prod_lt):
        intR += supcoef_lt / (i + 1) * (Tad_mid ** (i + 1) - T0 ** (i + 1))
    ...
    if not stop:
        ...
        for id in prod_ids:
            for coef in coefs1[id]:
                acoefs_prod_ht += coef * massfracs_r[id] * R_bar[id]

        for i, supcoef_ht in enumerate(acoefs_prod_ht):
            intR += supcoef_ht / (i + 1) * (Tad ** (i + 1) - T0 ** (i + 1))
